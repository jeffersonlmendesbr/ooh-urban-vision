...
const Tools = () => {
  ...
  const renderToolCard = (item: TimelineItem) => {
    ...
    return <HoverCard key={item.id}>
        <HoverCardTrigger asChild>
          <div className={cn('group relative p-4 rounded-lg border transition-all duration-300 cursor-pointer bg-white-pure border-ink-black/10 hover:border-gold-accent/40 hover:shadow-md hover:-translate-y-1 h-32 flex flex-col justify-between', isECA && 'ring-2 ring-gold-accent/30 bg-gradient-to-br from-gold-accent/5 to-gold-accent/10')}>
            {/* Icon */}
            <div className={cn('w-8 h-8 rounded-full flex items-center justify-center mb-2 transition-transform group-hover:scale-110', isECA ? 'bg-gold-accent text-ink-black' : 'text-ink-black bg-ink-black/5')}>
            ...
            <div className="flex-1 flex flex-col justify-between">
              <div>
                <h4 className={cn('font-heading text-sm font-medium text-ink-black mb-1 group-hover:text-gold-accent transition-colors leading-tight line-clamp-2', isECA && 'text-sm')}>
                  {item.title}
                </h4>
                
                {item.subtitle && <p className="text-xs text-gold-accent font-accent mb-2 line-clamp-1">
                    {item.subtitle}
                  </p>}
              </div>
              
              {item.year && <Badge className={cn('text-xs px-2 py-1 w-fit', isECA ? 'bg-gold-accent text-ink-black' : 'bg-ink-black/10 text-ink-black')}>
                  {item.year}
                </Badge>}

              {/* ECA Special Badge */}
              {isECA && <div className="absolute -top-1 -right-1">
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                {item.subtitle && <p className="text-xs text-gold-accent font-accent mb-2">
                    {item.subtitle}
                  </p>}
              </div>
            </div>
  ...
  return <SectionContainer id="tools" className="bg-gradient-editorial">
      <div className="max-w-7xl mx-auto px-6 w-full">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-6">
          {categories.map(category => <button key={category.key} onClick={() => setSelectedCategory(category.key)} className={cn('px-4 py-2 rounded-full text-sm font-accent transition-all duration-300', selectedCategory === category.key ? 'bg-gold-accent text-ink-black shadow-md' : 'bg-white-pure text-grey-sophisticated hover:bg-gold-accent/10 border border-ink-black/10')}>
              {category.title}
        ...
        <div className="grid grid-cols-5 gap-4 text-center pt-4 border-t border-gold-accent/20">
          {categories.map(category => <div key={category.key} className="cursor-pointer" onClick={() => setSelectedCategory(category.key)}>
